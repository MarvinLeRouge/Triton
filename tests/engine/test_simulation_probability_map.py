...
def test_simulation_exposes_probability_map_matching_grid_shape() -> None:
    sim, *_ = _make()
    assert sim.probability_maps[0].values.shape == (10, 10)
...
def test_advance_changes_probability_map_from_initial_prior() -> None:
    sim, *_ = _make()
    before = sim.probability_maps[0].values.copy()
    sim.advance()
    after = sim.probability_maps[0].values
...
def test_detection_concentrates_probability_on_detected_cell() -> None:
    ...
    sim.advance()
    assert sim.probability_maps[0].probability(5, 5) > 0.5
...
def test_probability_map_can_be_injected() -> None:
    ...
    sim, *_ = _make(probability_maps=[custom])
    assert sim.probability_maps[0] is custom
